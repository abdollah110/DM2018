...
//            if (mcStatus->at(ij)==23 || fabs(mcPID->at(ij))==13)              cout << mcPID->at(ij) <<"  "<< mcStatus->at(ij) <<" " << mcPt->at(ij) <<"  "<< mcMomPID->at(ij)<<  " " << mcGMomPID->at(ij)<<"\n";
                            if (fabs(mcPID->at(ij))==3  && mcStatus->at(ij)==23  && fabs(mcMomPID->at(ij))== 9000009 ) {numOf_s_quark++; thereisSquark= true;}
//                            cout <<" pdgid is "<< mcPID->at(ij)<<"\n";
                            if (fabs(mcPID->at(ij))==4  && mcStatus->at(ij)==23  && fabs(mcMomPID->at(ij))== 9000008 ) numOf_c_quark++;
//                            cout <<" pdgid is "<< mcPID->at(ij)<<"\n";
                        }
